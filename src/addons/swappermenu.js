...
// Electron 32 removed File.path — resolve via webUtils with a fallback.
const filePathOf = (file) => {
  try { return webUtils.getPathForFile(file); } catch (e) {}
  return file.path;
};
...
const editResourceSwapper = () => {
  ...
  soundFileInput?.addEventListener("change", (event) => {
    ...
    soundbuffer = null;
    soundFilePath = filePathOf(soundfile);
