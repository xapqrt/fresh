...
let customNotification = (data) => {
  if (typeof window.customNotification === "function") {
    return window.customNotification(data);
  }
};
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  const fetchAll = async () => {
    try {
      ...
      const fetchWithFallback = async (url, localFile) => {
        try {
          const res = await fetch(url);
          if (res.ok) return await res.json();
        } catch (e) {}
        return readLocal(localFile);
      };

      const [customizations, clan, ktiers] = await Promise.all([
        fetchWithFallback("https://raw.githubusercontent.com/zVipexx/dawn-client/refs/heads/main/badges.json", "badges.json"),
        fetchWithFallback("https://raw.githubusercontent.com/zVipexx/dawn-client/refs/heads/main/clans.json", "clans.json"),
        fetch("https://ktiers-production.up.railway.app/api/players").then((r) => r.json()).catch(() => []),
      ]);
  ...
  const formatLink = (link) => {
    if (!link) return "";
    let formatted = String(link).replace(/\\/g, "/");
    if (formatted.includes("raw.githubusercontent.com")) {
      formatted = formatted.replace("https://raw.githubusercontent.com/", "https://rawcdn.githack.com/");
    }
    return formatted;
  };
  ...
  const lobbyNews = async (settings) => {
    ...
    let news;
    try {
      news = await fetch("https://raw.githubusercontent.com/zVipexx/dawn-client/refs/heads/main/news.json").then((r) => r.json());
    } catch (e) {
      try {
        const p = path.join(__dirname, "../../news.json");
        if (fs.existsSync(p)) news = JSON.parse(fs.readFileSync(p, "utf8"));
      } catch (err) {}
    }
    if (!news || !Array.isArray(news) || !news.length) return;
  ...
  const loadTheme = () => {
    let addedStyles = document.getElementById("juice-styles-theme");
    if (!addedStyles) {
    ...
    let advancedStyles = document.getElementById("juice-styles-advanced");
    if (!advancedStyles) {
      advancedStyles = document.createElement("style");
      advancedStyles.id = "juice-styles-advanced";
    ...
    const updateTheme = (window.updateTheme = () => {
      ...
      if (settings.quickcss_enabled && settings.advanced_css) {
        advancedStyles.innerHTML = settings.advanced_css;
      } else {
  ...
  const handleServers = async () => {
    ...
    let mapImages = {};
    try {
      mapImages = await fetch("https://raw.githubusercontent.com/AwesomeSam9523/KirkaSkins/refs/heads/main/maps/full_mapimages.json").then((res) => res.json());
    } catch (e) {
      try {
        const p = path.join(__dirname, "../../maps.json");
        if (fs.existsSync(p)) mapImages = JSON.parse(fs.readFileSync(p, "utf8"));
      } catch (err) {}
    }

    if (mapImages && typeof mapImages === "object") {
      for (const key of Object.keys(mapImages)) {
        if (typeof mapImages[key] === "string" && !mapImages[key].includes("https")) {
  ...
  const handleClans = () => {
    async function fetchClan(clan) {
      try {
        ...
        console.warn("[Dawn] fetchClan error:", e);
        return null;
      }
    }
  ...
  customNotification = window.customNotification = (data) => {
  ...
  window.dumpCookies = async () => {
    try {
      const data = await ipcRenderer.invoke("dump-cookies");
      console.table(data);
      return data;
