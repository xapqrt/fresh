...
try {
  scriptsPath = ipcRenderer.sendSync("get-scripts-path");
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  // Patch health readout (About tab)
  try {
    window.__loadDawnPatchStatus = () => {
      const el = document.querySelector("#juice-menu #dawn-patch-status");
      if (!el) return;
  ...
  // performance_mode: strip menu transitions/animations/blur.
  try {
    const applyPerfMode = () => {
      const s = ipcRenderer.sendSync("get-settings");
      const el = document.querySelector("#juice-menu > .menu");
      if (el) el.classList.toggle('perf-mode', s ? s.performance_mode !== false : true);
    };
    window.__applyPerfMode = applyPerfMode;
    applyPerfMode();
  } catch (e) {}

  // F9 frame-time overlay
  try {
    let _ftOverlay = null, _ftActive = false, _ftHist = [], _ftRAF = null;
    const toggleFT = () => {
      _ftActive = !_ftActive;
      if (!_ftActive) {
        if (_ftRAF) { cancelAnimationFrame(_ftRAF); _ftRAF = null; }
        if (_ftOverlay) { _ftOverlay.remove(); _ftOverlay = null; }
        return;
      }
      if (!_ftOverlay) {
        _ftOverlay = document.createElement('div');
        Object.assign(_ftOverlay.style, {
          position: 'fixed', top: '8px', right: '8px',
          background: 'rgba(0,0,0,0.75)', color: '#0f0',
          fontFamily: 'monospace', fontSize: '12px',
          padding: '8px 12px', borderRadius: '4px',
          zIndex: '2147483646', pointerEvents: 'none', whiteSpace: 'pre',
        });
      ...
      const sample = (t) => {
        _ftHist.push(t - lastT); lastT = t;
        if (_ftHist.length > 120) _ftHist.shift();
        const n = _ftHist.length;
        if (n > 1) {
          let min = Infinity, max = -Infinity, sum = 0;
          for (const v of _ftHist) { if (v < min) min = v; if (v > max) max = v; sum += v; }
          _ftOverlay.textContent = `FT min ${min.toFixed(2)}ms  max ${max.toFixed(2)}ms  avg ${(sum / n).toFixed(2)}ms\nFPS ${(1000 / (sum / n)).toFixed(0)}  samples ${n}`;
        }
        if (_ftActive) _ftRAF = requestAnimationFrame(sample);
      };
      _ftRAF = requestAnimationFrame(sample);
    };
    document.addEventListener('keydown', (e) => {
      if (e.code === 'F9') { e.preventDefault(); toggleFT(); }
    });
  } catch (e) {}

  // Main process owns the keybind capture & F8 — drive the menu from IPC.
  try {
    ipcRenderer.on('toggle-menu', () => setTimeout(() => {
      if (window.__dawnMenu) window.__dawnMenu.toggle();
      if (window.__applyPerfMode) window.__applyPerfMode();
      if (window.__loadDawnPatchStatus) window.__loadDawnPatchStatus();
    }, 0));
  } catch (e) {}

  // Keep the local settings mirror fresh (bhop hook reads this exact object).
  try {
    ipcRenderer.on('settings-updated', (_, s) => { if (s && typeof s === 'object') Object.assign(settings, s); });
  } catch (e) {}
