...
class Menu {
  ...
  init() {
    const steps = [
      "setVersion", "setUser", "setKeybind", "dragMenu", "resizeMenu",
      "setLocalGradient", "setLocalBadges", "setLocalProfileBackground",
      "setTheme", "handleKeyEvents", "initMenu", "initChangelogs",
      "handleSliderInputs", "handleColorInputs", "handleMenuKeybindChange",
      "handleInspectKeybindChange", "handleMenuInputChanges",
      "handleMenuSelectChanges", "handleTabChanges", "handleInnerTabChanges",
      "handleSelectorChanges", "handleDropdowns", "handleAppearance",
      "bindWeaponOptions", "handleSearch", "handleButtons",
    ];
    for (const step of steps) {
      try { this[step](); } catch (e) {
        console.warn(`[Dawn] menu.${step} failed:`, e.message);
      }
    }

    try { initBrowser(this.menu); } catch (e) {
      console.warn("[Dawn] initBrowser failed:", e.message);
    }
  ...
  updateGlobalWeaponConfig() {
    ...
    this.notifyWeaponConfig();
  }

  notifyWeaponConfig() {
    try { document.dispatchEvent(new CustomEvent("dawn-weapon-config-updated")); } catch (e) {}
  }
  ...
  toggle(force) {
    ...
    this.menuToggle.setAttribute("data-active", next);
    this.localStorage.setItem("juice-menu", next);
  }
  ...
  handleSliderInputs() {
    const sliderMap = [
      ...
      {
        slider: document.getElementById("bhop_engage_ms"),
        input: document.querySelector(".bhop-engage-value"),
      }
    ];
